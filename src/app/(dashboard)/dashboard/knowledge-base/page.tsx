...
export default function KnowledgeBasePage() {
  ...
  // Funkcja pobierająca pełne dane zapisanych artykułów z data.ts
  const refreshSavedArticles = () => {
    ...
    if (savedIdsString) {
      ...
      // Przeszukujemy wszystkie kategorie w poszukiwaniu artykułów o tych ID
  ...
  const removeSaved = (id: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Zapobiega otwarciu kategorii
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {savedArticles.map((art) => (
              <div 
                key={art.id}
                onClick={() => {
                  setSearchTerm(art.title); // Ustawiamy tytuł artykułu jako frazę wyszukiwania
                  setActiveCategory(art.categorySlug); // Przechodzimy do kategorii
                }}
